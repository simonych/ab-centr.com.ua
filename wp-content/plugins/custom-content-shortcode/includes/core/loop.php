...
class CCS_Loop {
  ...
  function __construct() {
    ...
    add_ccs_shortcode( array(
      ...
      'the-pagination' => array( $this, 'default_pagination_shortcode'),
    ));
        ...
        if (!empty($parameters['taxonomy'.$suffix])) {
          ...
          $args = array(
            ...
            'compare' => !empty($parameters['compare'.$suffix]) ?
              strtoupper( $parameters['compare'.$suffix] ) : 'IN'
          );
 ...
 */
  ...
  function default_pagination_shortcode() {
    ob_start();
    if (function_exists('bootstrapify_pagination')) {
      bootstrapify_pagination();
    } else {
      wp_default_pagination();
    }
    return ob_get_clean();
  }
