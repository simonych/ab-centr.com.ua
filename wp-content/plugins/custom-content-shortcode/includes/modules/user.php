	...
	public static function user_shortcode( $atts ) {
      ...
      'out' => ''
		...
		switch ( $field ) {
			...
			case 'archive-link':
				...
				$text = !empty($text) ? $text : $current_user->display_name;
        ...
        if ($out=='slug') {
          ...
          return rtrim(implode(',',array_map('ucwords', $current_user->roles)),',');
			...
			case 'edit-url':
				return admin_url( 'user-edit.php?user_id=' . $current_user->ID );
			break;

			case 'edit-link':
				if (empty($text)) $text = 'Edit Profile';
				$url = admin_url( 'user-edit.php?user_id=' . $current_user->ID );
				return '<a href="'.$url.'">'.$text.'</a>';
      ...
      case 'browser':
        ...
        break;
			default:
				// Custom user field
				$result = get_user_meta( $current_user->ID, $field, true );
				break;
		}
