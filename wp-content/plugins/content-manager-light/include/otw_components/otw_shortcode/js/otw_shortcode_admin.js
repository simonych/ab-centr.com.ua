...
otw_shortcode_object.prototype.init_dropdown_actions = function(){
	...
	with( this ){
		...
		jQuery( 'body' ).click( function( event ){
			...
			if( jQuery( event.target ).parents( '.mce_otwshortcodebtn' ).size() ){
				close_it = false;
			};
						...
						default:
								jQuery.get( 'admin-ajax.php?action=otw_shortcode_editor_dialog&shortcode=' + matches[1],function(b){
									...
									tb_position = function(){
										...
										jQuery( '#TB_ajaxContent' ).scrollTop( 0 );
...
otw_shortcode_editor_object.prototype.live_reload = function(){
	...
	with( this ){
		...
		jQuery.post( 'admin-ajax.php?action=otw_shortcode_live_reload&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
otw_shortcode_editor_object.prototype.live_preview = function(){
	...
	jQuery.post( 'admin-ajax.php?action=otw_shortcode_live_preview&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
		...
		jQuery( '#otw-shortcode-preview' ).height( jQuery( '#otw-shortcode-preview' ).contents().find('body').outerHeight( true ) + 50 );
		
		jQuery( '#TB_ajaxContent' ).scroll( function(){
			
			
			var cellHeight = jQuery( '#otw-shortcode-preview' ).parents( '.otw-shortcode-editor-preview-container' ).parent().height();
			
			var newPaddingTop = this.scrollTop;
			
			
			if( ( newPaddingTop + jQuery( '#otw-shortcode-preview' ).parents( '.otw-shortcode-editor-preview-container' ).height() ) <  cellHeight )
			{
