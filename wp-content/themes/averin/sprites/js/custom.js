...
jQuery(document).ready(function(){
	...
	$(".content").find("p:not(.separator)").each(function(){
	 ...
	 if($('.toggle-switch').next('.kwicks-wrapper').length>0 || $('.toggle-switch').next('.scrollable').length>0 )
	  $('.toggle-switch').css("bottom",39);
	 
	if($('.slider-wrapper-shade').find('.stageslider').length>0) $('.toggle-switch').css("bottom",39);
...
$(".slider-wrapper-shade>div").slideUp('normal',function(){
	
	$(".toggle-switch").animate({bottom:40},'fast');
	}); $(this).addClass('inactive').removeClass('active'); 

 },function(){ 
 
 if($(this).next('.kwicks-wrapper').length>0 || $(this).next('.scrollable').length>0 || $(this).next('.stagableSlider').length>0)
  $(this).css("bottom",39).addClass('active').removeClass('inactive');
 else
  $(this).css("bottom",69).addClass('active').removeClass('inactive');
 $(".slider-wrapper-shade>div").slideDown('normal'); 
...
if(jQuery(".stageslider").length>0) {
 jQuery('.stageslider').stagableSlider();
...
var cflag= true,testw = $(window).width();

$(window).resize(function(){ 

if( $(window).width() < testw )
 { cflag=false ; scroll_button.hide(); } 
 else 
 { cflag=true ; scroll_button.show(); }
   });
