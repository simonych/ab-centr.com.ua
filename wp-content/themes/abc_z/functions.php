 ...
 *
*/
function kama_excerpt( $args = array() ){
	global $post;
	...
	if( ! $text ){
		...
		if( ! $post->post_excerpt && strpos( $post->post_content, '<!--more-->') ){
			...
			$text = preg_replace( "~\n\n+~s", "</p><p>", $text );
			$text = '<p>'. str_replace( "\n", '<br />', $text ) .' <a href="'. get_permalink( $post->ID ) .'#more-'. $post->ID .'" class="moreInfo">'. $more_text .'</a></p>';
	...
	$text .= '<a href="'. get_permalink( $post->ID ) .'#more-'. $post->ID .'" class="moreInfo">'. $more_text .'</a>';
