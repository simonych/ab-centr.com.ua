...
function art_menu_xml_parcer($content, $show_sub_menus, $menu_class) {
  ...
  foreach($ul_elements as $ul) {
    ...
    if (($ul->getAttribute('class') == "art-vmenu") || ($ul->getAttribute('class') == "art-hmenu"))
        break;
    continue;
  }
...
function art_hmenu_output($content) {
  $menu = art_menu_worker($content, true, 'art-hmenu');
...
function art_vmenu_output($subject, $content) {
  ...
  $result = '';
  $vmenu =	art_menu_worker($content, true, 'art-vmenu');
  $result .= <<< EOT
